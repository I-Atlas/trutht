...
OPERATIONS = {
    ...
    'not_and':     (lambda x, y: not (x and y)),

    'imp':  (lambda x, y: (not x) or y),
    'r_imp':  (lambda x, y: not y or x),

    'equ':        (lambda x, y: x == y),
    'r_equ':       (lambda x, y: x != y),
}
...
def string_to_bool(string):
    # Converts a string to boolean if string is either 'True' or 'False' otherwise returns it unchanged.
...
def group_operations(phrase):
    ...
    # Recursively groups logical operations into seperate lists based on the order of operations such that each list
    # is one operation.

    # Order of operations is: not, and, or, implication
...
class Truths:
    ...
    def valuation(self, col_number=-1):
        ...
        # Evaluates an expression in a table column as a tautology, a contradiction or a contingency
